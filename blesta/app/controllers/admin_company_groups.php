...
class AdminCompanyGroups extends AppController
{
    ...
    private $client_group_fields = [
        ...
        'void_inv_canceled_service_days', 'prevent_unverified_payments', 'unique_contact_emails',
        'force_email_usernames', 'email_verification', 'required_contact_fields', 'shown_contact_fields',
        'clients_increment', 'clients_start', 'clients_format'
    ];
    ...
    public function add()
    {
        ...
        if (!empty($this->post)) {
            ...
            // Default and serialize required_contact_fields
            if (empty($this->post['required_contact_fields'])) {
                $this->post['required_contact_fields'] = [];
            }
            if (empty($this->post['shown_contact_fields'])) {
                $this->post['shown_contact_fields'] = [];
            }

            $white_list = ['company', 'title', 'address1', 'address2', 'city', 'country', 'state', 'zip', 'phone', 'fax'];
            foreach ($this->post['required_contact_fields'] as $key => $field) {
                if (!in_array($field, $white_list)) {
                    unset($this->post['required_contact_fields'][$key]);
                }
            }
            $temp_contact_fields = $this->post['required_contact_fields'];
            $this->post['required_contact_fields'] = base64_encode(
                serialize((array)$this->post['required_contact_fields'])
            );

            foreach ($this->post['shown_contact_fields'] as $key => $field) {
                if (!in_array($field, $white_list)) {
                    unset($this->post['shown_contact_fields'][$key]);
                }
            }
            $temp_shown_contact_fields = $this->post['shown_contact_fields'];
            $this->post['shown_contact_fields'] = base64_encode(
                serialize((array)$this->post['shown_contact_fields'])
            );
            ...
            if (($errors = $this->ClientGroups->errors())) {
                ...
                // Reset the posted delivery methods and required contact fields
                $this->post['delivery_methods'] = $temp_delivery_methods;
                $this->post['required_contact_fields'] = $temp_contact_fields;
                $this->post['shown_contact_fields'] = $temp_shown_contact_fields;
        ...
        if (!isset($settings) || (isset($vars->use_company_settings) && $vars->use_company_settings == 'true')) {
            ...
            $vars->required_contact_fields = (isset($settings['required_contact_fields'])
                ? unserialize(base64_decode($settings['required_contact_fields']))
                : []
            );
            $vars->shown_contact_fields = (isset($settings['shown_contact_fields'])
                ? unserialize(base64_decode($settings['shown_contact_fields']))
                : []
            );
            $vars->force_email_usernames = isset($settings['force_email_usernames'])
                ? $settings['force_email_usernames']
                : 'false';
            $vars->email_verification = isset($settings['email_verification'])
    ...
    public function edit()
    {
        ...
        $vars->use_company_settings = (isset($use_company_settings)
            ? $use_company_settings
            : $vars->use_company_settings
        );
