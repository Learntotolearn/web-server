...
class Enom extends RegistrarModule
{
    ...
    public function getFilteredTldPricing($module_row_id = null, $filters = [])
    {
        ...
        if (!isset($pricing)) {
            ...
            $response = $all->PE_GetRetailPricing(['TLDOnly' => 1]);
        ...
        $tld_yearly_prices = [];
        if (!empty($pricing->pricestructure->tld) > 0) {
            $currencies = $this->Currencies->getAll(Configure::get('Blesta.company_id'));
            foreach ($pricing->pricestructure->tld as $tld_data) {
                $tld = '.' . $tld_data->tld;
                ...
                foreach ($currencies as $currency) {
                    ...
                    $register_price =  $this->Currencies->convert(
                            $tld_data->registerprice,
                            'USD',
                            $currency->code,
                            Configure::get('Blesta.company_id')
                        );
                    $transfer_price = $tld_data->transferenabled == 'true'
                        ? ($this->Currencies->convert(
                                $tld_data->transferprice,
                                'USD',
                                $currency->code,
                                Configure::get('Blesta.company_id')
                            )
                        ...
                        : null;
                    $renewal_price = $tld_data->renewenabled == 'true'
                        ? ($this->Currencies->convert(
                                $tld_data->renewprice,
                                'USD',
                                $currency->code,
                    ...
                    foreach (range(1, 10) as $years) {
                        ...
                        $tld_yearly_prices[$tld][$currency->code][$years] = [
                            ...
                            'transfer' => ($transfer_price === null ? null : $transfer_price * $years),
                            'renew' => ($renewal_price === null ? null : $renewal_price * $years),
                        ];
                    }
                }
            }
